...
constexpr uint32_t PCAP_MAGIC_NANO = 0xa1b23c4d;     // Nanosecond native
constexpr uint32_t PCAP_MAGIC_NANO_SWAPPED = 0x4d3cb2a1; // Nanosecond swapped
...
bool PcapReader::open(const std::string& filename) {
    ...
    if (global_header_.magic_number == PCAP_MAGIC_NATIVE || 
        global_header_.magic_number == PCAP_MAGIC_NANO) {
        needs_byte_swap_ = false;
    } else if (global_header_.magic_number == PCAP_MAGIC_SWAPPED || 
               global_header_.magic_number == PCAP_MAGIC_NANO_SWAPPED) {
